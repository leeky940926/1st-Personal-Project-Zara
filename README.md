...
최초 초안 기획부터 구현까지 혼자했으며, EC2나 RDS에 대해 배포를 해 본 경험이 있지만

이번 프로젝트에서 배포는 비용 문제상 하지 않을 예정입니다.
...
5. ORM최적화를 고민하게 된 상품상세페이지

Path Paramter : product_id

특정 상품의 상세페이지를 조회하는데, 우선 연관 테이블의 데이터를 가져와서 쿼리 발생을 줄이기 위해 select_related와 prefetch_related를 사용했습니다.

색상 3개와 사이즈 3개의 경우의 수 총합은 9가지인데, 9번의 반복문을 돌리자니 색상은 3번만 돌려서 가져오면 되기 때문에 비효율적이라고 판단했습니다.
